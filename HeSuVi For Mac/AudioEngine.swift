...
class AudioEngine {
    ...
    func setupFilterAU(completion: @escaping (Result<Bool, Error>) -> Void) {
        DispatchQueue.global(qos: .default).async {
            let description = AudioComponentDescription(componentType: 0x61756678, // "aufx"
                                                        componentSubType: 0x68737666, // "hsvf"
                                                        componentManufacturer: 0x4c53636f, // "LSco"
                                                        componentFlags: 0,
            ...
            if components.count != 1 {
                ...
                DispatchQueue.main.async {
                    completion(.failure(HeSuViError.missingAU))
                }
                return
            }
